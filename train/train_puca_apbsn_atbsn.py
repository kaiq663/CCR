...
def main(opt, opt_apbsn, opt_puca, opt_atbsn):
    train_loaders = []
    ...
    Model = getattr(__import__('model'), opt_atbsn['model'])
    model_atbsn = Model(opt_atbsn)
    model_atbsn.data_parallel()
    ...
    def train_step(data):
        output_atbsn = model_apbsn.validation_step(data)
        ...
        model.train_step(data, output_puca_2, output_puca_1, output_apbsn=output_apbsn, output_atbsn=output_atbsn)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_atbsn", type=str, default='option/atbsn_lite.json')


    argspar = parser.parse_args()

    opt = parse(argspar.config)
    opt_atbsn = parse(argspar.config_atbsn)
    opt_apbsn = parse(argspar.config_apbsn)
    opt_puca = parse(argspar.config_puca)
    recursive_log(opt['log_file'], opt)

    main(opt, opt_apbsn, opt_puca, opt_atbsn)