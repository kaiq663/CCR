...
def curriculum_weight(difficulty):
    cl_lambda = 0.25
    #SIDD PUCAkb1 APBSN SASL ATBSN-D
    diff_sidd_list = [32.59, 36.74, 37.39,  37.84]

    #APBSN ATBSN-D SASL PUCAkb1
    diff_ood_list = [36.95, 37.51, 37.67, 37.79]

    diff_sidd_list = [32.59, 36.74]
    weights = [(1 + cl_lambda) if difficulty > x else (1 - cl_lambda) for x in diff_sidd_list]
    weights.append(len(diff_sidd_list))
...
class ATBSN_Curricular_Model(BaseModel):
    def __init__(self, opt):
        super(ATBSN_Curricular_Model, self).__init__(opt)
        self.stage = None
    ...
    def criterion_curricular_original(self, a, p, n1, n2, n3, n4, n5, n6, inp, weight):
        ...
        for i in range(len(a_vgg)):
            ...
            d_an1 = self.criteron(a_vgg[i], n1_vgg[i].detach())
            d_an2 = self.criteron(a_vgg[i], n2_vgg[i].detach())
            d_an3 = self.criteron(a_vgg[i], n3_vgg[i].detach())
            d_an4 = self.criteron(a_vgg[i], n4_vgg[i].detach())
            d_an5 = self.criteron(a_vgg[i], n5_vgg[i].detach())
            d_an6 = self.criteron(a_vgg[i], n6_vgg[i].detach())
            d_inp = self.criteron(a_vgg[i], inp_vgg[i].detach())
            contrastive = d_ap / (
    ...
    def criterion_curricular_1(self, a, p, n1, inp, weight):
        a_vgg, p_vgg, n1_vgg = self.vgg(a), self.vgg(p), self.vgg(n1)
        inp_vgg = self.vgg(inp)
        n1_weight, inp_weight = weight
        loss = 0
        for i in range(len(a_vgg)):
            d_ap = self.criteron(a_vgg[i], p_vgg[i].detach())
            d_an1 = self.criteron(a_vgg[i], n1_vgg[i].detach())
            d_inp = self.criteron(a_vgg[i], inp_vgg[i].detach())
            contrastive = d_ap / (d_an1 * n1_weight + d_inp * inp_weight + 1e-7)
            loss += self.weights[i] * contrastive
        return loss

    def criterion_curricular_2(self, a, p, n1, n2, inp, weight):
        a_vgg, p_vgg, n1_vgg, n2_vgg = self.vgg(a), self.vgg(p), self.vgg(n1), self.vgg(n2)
        inp_vgg = self.vgg(inp)
        n1_weight, n2_weight, inp_weight = weight
        loss = 0
        for i in range(len(a_vgg)):
            d_ap = self.criteron(a_vgg[i], p_vgg[i].detach())
            d_an1 = self.criteron(a_vgg[i], n1_vgg[i].detach())
            d_an2 = self.criteron(a_vgg[i], n2_vgg[i].detach())
            d_inp = self.criteron(a_vgg[i], inp_vgg[i].detach())
            contrastive = d_ap / (d_an1 * n1_weight + d_an2 * n2_weight + d_inp * inp_weight + 1e-7)
            loss += self.weights[i] * contrastive
        return loss

    def criterion_curricular_3(self, a, p, n1, n2, n3, inp, weight):
        a_vgg, p_vgg, n1_vgg, n2_vgg, n3_vgg = self.vgg(a), self.vgg(p), self.vgg(n1), self.vgg(n2), self.vgg(n3)
        inp_vgg = self.vgg(inp)
        n1_weight, n2_weight, n3_weight, inp_weight = weight
        loss = 0
        for i in range(len(a_vgg)):
            d_ap = self.criteron(a_vgg[i], p_vgg[i].detach())
            d_an1 = self.criteron(a_vgg[i], n1_vgg[i].detach())
            ...
            d_an3 = self.criteron(a_vgg[i], n3_vgg[i].detach())
            d_inp = self.criteron(a_vgg[i], inp_vgg[i].detach())
            contrastive = d_ap / (d_an1 * n1_weight + d_an2 * n2_weight + d_an3 * n3_weight + d_inp * inp_weight + 1e-7)
            loss += self.weights[i] * contrastive
        return loss

    def train_step(self, data, output_puca_2, output_puca_1, output_atbsn=None, output_apbsn=None):
        self.iter += 1
        self.update_stage()
        ...
        if self.stage == 'BNN':
            ...
            self.loss_l1 = self.criteron(unet, output_puca_2) + self.criteron(unet, output_puca_1)
            self.loss_cr = self.criterion_curricular_1(unet, output_puca_2, output_puca_1, input, weight=[0.5, 0.5])
            self.loss = self.loss_l1 + 0.75*self.loss_cr
            self.loss.backward()
