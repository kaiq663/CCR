...
def get_conv(ratio=5, stride=1, device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    conv = torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=ratio, stride=stride, padding=0, bias=False)
    conv.weight = torch.nn.Parameter((torch.ones((1,1,ratio, ratio))/(1.0 * ratio * ratio)).to(device))
    return conv

def smooth(noisy, ratio=13, stride=1, device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    conv = get_conv(ratio, stride,device=device)
    b, c, h, w = noisy.shape
...
def calculate_sliding_std(img, upsampler, kernel_size, stride, device):
    slided_mean = smooth(img, kernel_size, stride=stride, device=device)
    mean_upsampled = upsampler(slided_mean)
    variance = smooth( (img - mean_upsampled)**2, kernel_size, stride=stride, device=device)
