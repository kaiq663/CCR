...
class AFM_B(nn.Module):
    def __init__(self, fq_bound=1.0):
        ...
        self.conv1 = nn.Conv2d(in_channels=12, out_channels=16, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True).cuda()
        self.down1 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True).cuda()
        self.down2 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True).cuda()
    ...
    def forward(self, clean, noisy):
        ...
        ####################################################
        filter_input = torch.cat(
            [noisy, torch.log10(torch.abs(noisy_fq) + 1), clean, torch.log10(torch.abs(clean_fq) + 1)], dim=1)
